...
CI (`.github/workflows/ci.yml`) runs `npx astro sync`, `npm run lint`, then `npm run build` on every push/PR to `master`; the build step needs `SUPABASE_URL`/`SUPABASE_KEY` repo secrets.

There is no test runner configured yet — no `npm test` script exists.
