...
This plan is mirrored at `context/deployment/deploy-plan.md`, the canonical location per `CLAUDE.md`'s Module 1 Lesson 5 hand-off contract (read by downstream milestone-planning skills as "what's already deployed"). This `changes/` copy is kept for in-flight change tracking per explicit user request. If the two drift, `context/deployment/deploy-plan.md` is the source of truth.
