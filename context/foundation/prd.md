...
- FR-004: System outputs a recommended packing order as a lightweight grid/diagram plus text (not full 2D/3D rendering) for the goods that fit. Priority: must-have
  > Socrates: Counter-argument considered: "text alone is hard to trust or execute without any visual reference." Resolution: revised — output includes a simple grid/diagram alongside the text order, short of the full 2D/3D visualization already cut from MVP scope.
...
- **No 2D/3D visual load rendering.** The packing order is communicated as text plus a lightweight grid diagram (FR-004), not a full visual/3D renderer. Cut during MVP scoping; explicit v2 candidate.
- **No weight/max-load constraint checking.** v1 calculates fit by volume only. Cut during MVP scoping alongside visualization; explicit v2 candidate.
...
None — all sections were captured during discovery (`/10x-shape`), and the closing quality cross-check passed with no gaps.
